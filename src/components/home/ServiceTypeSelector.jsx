...
function ServiceTypeSelector({ 
  ...
  const getServiceBadge = (product) => {
    if (product.auto_tag) return <span className="service-badge auto">자동</span>
    if (product.name.includes('인기게시물')) return <span className="service-badge new">B</span>
    return null
  }

  const shouldShowTabs = () => {
    if (products.length === 0) return false
    const hasKorean = products.some(p => p.name.includes('한국인'))
    const hasForeign = products.some(p => p.name.includes('외국인'))
    return hasKorean && hasForeign
  }
